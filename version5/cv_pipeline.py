...
def run_cv(image_path: str) -> Dict:
    try:
        ...
        if object_type != "PCB" or cropped is None or mask is None:
            return {
        ...
        candidates = detect_component_candidates(preprocessed, mask) or []
        ...
        components = heuristic_classification(components or [])
        components = mark_ic_candidates(components or [])
...
def preprocess_pcb(pcb_image: np.ndarray, mask: np.ndarray) -> Optional[np.ndarray]:
    if pcb_image is None or mask is None:
        return None
...
def detect_component_candidates(preprocessed: np.ndarray, mask: np.ndarray):
    try:
        if preprocessed is None or mask is None:
            return []
...
def extract_features(
    ...
    if preprocessed is None or pcb_image is None:
        return []
    ...
    try:
        ...
        for i, box in enumerate(boxes):
            try:
                ...
                if region_gray.size == 0 or w * h == 0:
                    continue
...
def mark_ic_candidates(components):
    ...
    try:
        sorted_comps = sorted(components, key=lambda x: x.get("area", 0), reverse=True)
...
def heuristic_classification(components: List[Dict]) -> List[Dict]:
    for comp in components:
        try:
            ar = comp.get("aspect_ratio", 0)
            size = comp.get("size")
            edges = comp.get("edge_density", 0)
